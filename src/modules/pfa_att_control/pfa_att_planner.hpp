...
protected:
		...
		float & out_theta, Dcmf & out_R)
	{
		...
		if (!is_attainable(R_r, f_r)) {
			...
			const bool is_b3r_and_f_r_almost_parallel = ((b3r.dot(f_r) / f_r.norm()) < -0.99f);
...
private:
	...
	void updateParams() override
	{
		...
		_param.sigma_a = _param_servo_x_angle_max.get();
		_param.sigma_b = _param_servo_y_angle_max.get();
